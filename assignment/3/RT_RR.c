...
int main() {
    while(1) {
        ...
        if(scanNum == 1) {          // cfs default
            int status;
            char *compileCommand = "gcc -o cfsd cfsd.c -lm"; // 컴파일 명령어
            // CPU 1개만 사용
            char *executionCommand = "taskset 0x1 ./cfsd"; // 실행 명령어
            ...
            int status;
            char *compileCommand = "gcc -o cfsn cfsn.c -lm"; // 컴파일 명령어
            char *executionCommand = "./cfsn"; // 실행 명령어
